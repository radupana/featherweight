...
class WorkoutDataModelTest {
    ...
    fun `WorkoutStatus enum has expected values`() {
        assertThat(WorkoutStatus.values()).hasLength(3)
        assertThat(WorkoutStatus.values()).asList().contains(WorkoutStatus.NOT_STARTED)
        assertThat(WorkoutStatus.values()).asList().contains(WorkoutStatus.IN_PROGRESS)
        assertThat(WorkoutStatus.values()).asList().contains(WorkoutStatus.COMPLETED)
    }
    ...
    fun `Workout with different status transitions`() {
        ...
        val completedWorkout = inProgressWorkout.copy(
            status = WorkoutStatus.COMPLETED,
            durationSeconds = 4500L
        )
        assertThat(completedWorkout.status).isEqualTo(WorkoutStatus.COMPLETED)
        assertThat(completedWorkout.durationSeconds).isEqualTo(4500L)
    }
    ...
    fun `SetLog stores all required fields`() {
        ...
        assertThat(setLog.id).isEqualTo(1L)
        assertThat(setLog.exerciseLogId).isEqualTo(100L)
    ...
    fun `SetLog order maintains sequence`() {
        val sets = listOf(
            SetLog(id = 1L, exerciseLogId = 100L, setOrder = 0),
            SetLog(id = 2L, exerciseLogId = 100L, setOrder = 1),
            SetLog(id = 3L, exerciseLogId = 100L, setOrder = 2)
        )
    ...
    fun `ExerciseLog stores all required fields`() {
        ...
        assertThat(exerciseLog.id).isEqualTo(1L)
        assertThat(exerciseLog.workoutId).isEqualTo(50L)
        assertThat(exerciseLog.exerciseVariationId).isEqualTo(100L)
    ...
    fun `ExerciseLog handles superset grouping`() {
        ...
        val superset2 = ExerciseLog(
            id = 2L,
            workoutId = 50L,
            exerciseVariationId = 101L,
            exerciseOrder = 1,
            supersetGroup = 1
        )
    ...
    fun `ExerciseLog handles exercise swapping`() {
        val swappedExercise = ExerciseLog(
            id = 3L,
            workoutId = 50L,
            exerciseVariationId = 200L, // New exercise
            exerciseOrder = 2,
            originalVariationId = 100L, // Original exercise
            isSwapped = true
        )
        
        assertThat(swappedExercise.isSwapped).isTrue()
        assertThat(swappedExercise.originalVariationId).isEqualTo(100L)
        assertThat(swappedExercise.exerciseVariationId).isEqualTo(200L)
    }

    @Test
    fun `ExerciseLog maintains exercise order`() {
        val exercises = listOf(
            ExerciseLog(id = 1L, workoutId = 50L, exerciseVariationId = 100L, exerciseOrder = 0),
            ExerciseLog(id = 2L, workoutId = 50L, exerciseVariationId = 101L, exerciseOrder = 1),
            ExerciseLog(id = 3L, workoutId = 50L, exerciseVariationId = 102L, exerciseOrder = 2),
            ExerciseLog(id = 4L, workoutId = 50L, exerciseVariationId = 103L, exerciseOrder = 3)
        )
        
        assertThat(exercises.map { it.exerciseOrder }).isInOrder()
        assertThat(exercises.all { it.workoutId == 50L }).isTrue()
    }
    ...
    fun `PendingOneRMUpdate contains all required fields`() {
        ...
        assertThat(update.exerciseVariationId).isEqualTo(1L)
    ...
    fun `ParseRequest tracks parsing status`() {
        val now = LocalDateTime.now()
        val pendingRequest = ParseRequest(
            id = 1L,
        ...
        val completedRequest = pendingRequest.copy(
    ...
    fun `ExerciseSwapHistory records swap details`() {
        ...
        assertThat(swap.originalExerciseId).isEqualTo(100L)
        assertThat(swap.swappedToExerciseId).isEqualTo(200L)
        assertThat(swap.workoutId).isEqualTo(50L)
        assertThat(swap.programmeId).isEqualTo(10L)
