...
class ProgrammeRPEIntegrationTest {
    ...
    @Test
    fun `SetLog is created with correct targetRpe from WorkoutStructure`() = runTest {
        // Given a workout structure with RPE values
        val exerciseStructure = ExerciseStructure(
    ...
    @Test
    fun `RPE values are preserved when some sets have null RPE`() = runTest {
    ...
    fun `RPE list is null when no RPE values are provided`() {
        ...
        val rpeList = sets.map { it["rpe"] as? Double }
            .map { it?.toFloat() }
