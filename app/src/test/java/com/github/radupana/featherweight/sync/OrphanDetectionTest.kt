...
class OrphanDetectionTest {
    ...
    @Test
    fun `orphan detection - exercise log without parent workout is skipped`() =
        runTest {
            val userId = "test-user"
            val orphanedExerciseLog =
                FirestoreExerciseLog(
                    localId = "orphan-ex-log",
            ...
            val validExerciseLog =
                FirestoreExerciseLog(
                    localId = "valid-ex-log",
                    workoutId = "workout-1",
                    exerciseId = "exercise-1",
                    exerciseOrder = 1,
                    notes = null,
            ...
            coEvery {
                firestoreRepository.downloadExerciseLogs(
                    userId,
                    null,
                )
            } returns Result.success(listOf(orphanedExerciseLog, validExerciseLog))
            mockAllFirestoreUploads()

            val result = syncManager.syncAll()

            assertThat(result.isSuccess).isTrue()
            coVerify(exactly = 1) { exerciseLogDao.upsertExerciseLog(any()) }
            coVerify {
                exerciseLogDao.upsertExerciseLog(
                    withArg {
                        assertThat(it.id).isEqualTo("valid-ex-log")
                    },
                )
            }
        }
    ...
    @Test
    fun `orphan detection - set log without parent exercise log is skipped`() =
        runTest {
            val userId = "test-user"
            val orphanedSetLog =
                FirestoreSetLog(
                    localId = "orphan-set",
                    exerciseLogId = "non-existent-exercise-log",
                    setOrder = 1,
                    targetReps = 10,
                    targetWeight = 100f,
                    actualReps = 10,
            ...
            coEvery {
                exerciseLogDao.getExistingExerciseLogIds(
                    listOf(
                        "non-existent-exercise-log",
                        "exercise-log-1",
                    ),
                )
            } returns listOf("exercise-log-1")
            coEvery { setLogDao.upsertSetLog(any()) } returns Unit
            coEvery { firestoreRepository.downloadSetLogs(userId, null) } returns Result.success(listOf(orphanedSetLog, validSetLog))

            val result = syncManager.syncAll()

            assertThat(result.isSuccess).isTrue()
            coVerify(exactly = 1) { setLogDao.upsertSetLog(any()) }
            coVerify {
                setLogDao.upsertSetLog(
                    withArg {
                        assertThat(it.id).isEqualTo("valid-set")
                    },
                )
            }
        }
    ...
    @Test
    fun `orphan detection - template exercise without parent template is skipped`() =
        runTest {
            val userId = "test-user"
            val orphanedTemplateExercise =
                FirestoreTemplateExercise(
                    localId = "orphan-template-ex",
                    userId = userId,
            ...
            val validTemplateExercise =
                FirestoreTemplateExercise(
                    localId = "valid-template-ex",
                    userId = userId,
            ...
            // Override specific mocks after general mocks
            coEvery { workoutDao.getAllWorkouts(userId) } returns emptyList()
            coEvery { workoutTemplateDao.getTemplates(userId) } returns listOf(template)
            coEvery { templateExerciseDao.upsertTemplateExercise(any()) } returns Unit
            coEvery {
                firestoreRepository.downloadTemplateExercises(
                    userId,
                    null,
                )
            } returns Result.success(listOf(orphanedTemplateExercise, validTemplateExercise))

            val result = syncManager.syncAll()

            assertThat(result.isSuccess).isTrue()
            coVerify(exactly = 1) { templateExerciseDao.upsertTemplateExercise(any()) }
            coVerify {
                templateExerciseDao.upsertTemplateExercise(
                    withArg {
                        assertThat(it.id).isEqualTo("valid-template-ex")
                    },
                )
            }
        }
    ...
    @Test
    fun `orphan detection - template set without parent template exercise is skipped`() =
        runTest {
            val userId = "test-user"
            val orphanedTemplateSet =
                FirestoreTemplateSet(
                    localId = "orphan-template-set",
                    userId = userId,
                    templateExerciseId = "non-existent-template-exercise",
                    setOrder = 1,
                    targetReps = 10,
                    targetWeight = 100f,
                    targetRpe = null,
            ...
            val validTemplateSet =
                FirestoreTemplateSet(
                    localId = "valid-template-set",
                    userId = userId,
            ...
            coEvery { workoutDao.getAllWorkouts(userId) } returns emptyList()
            coEvery { workoutTemplateDao.getTemplates(userId) } returns listOf(template)
            coEvery { templateExerciseDao.getExercisesForTemplate("template-1") } returns emptyList()
            coEvery { templateSetDao.upsertTemplateSet(any()) } returns Unit
            mockAllDaoMethods()
            mockAllFirestoreDownloads()
            coEvery {
                firestoreRepository.downloadTemplateSets(
                    userId,
                    null,
                )
            } returns Result.success(listOf(orphanedTemplateSet, validTemplateSet))
            mockAllFirestoreUploads()

            val result = syncManager.syncAll()

            assertThat(result.isSuccess).isTrue()
            coVerify(exactly = 0) { templateSetDao.upsertTemplateSet(any()) }
        }

    @Test
    fun `orphan detection - cascade effect when parent is deleted locally but child exists remotely`() =
        runTest {
            val userId = "test-user"
            val orphanedExerciseLog =
                FirestoreExerciseLog(
            ...
            every { authManager.getCurrentUserId() } returns userId
            coEvery { workoutDao.getAllWorkouts(userId) } returns emptyList()
            coEvery { exerciseLogDao.getExistingExerciseLogIds(listOf("orphan-ex-log")) } returns emptyList()
            coEvery { exerciseLogDao.upsertExerciseLog(any()) } returns Unit
            coEvery { setLogDao.upsertSetLog(any()) } returns Unit
            mockAllDaoMethods()
            mockAllFirestoreDownloads()
            coEvery { firestoreRepository.downloadExerciseLogs(userId, null) } returns Result.success(listOf(orphanedExerciseLog))
            coEvery { firestoreRepository.downloadSetLogs(userId, null) } returns Result.success(listOf(orphanedSetLog))
            mockAllFirestoreUploads()

            val result = syncManager.syncAll()

            assertThat(result.isSuccess).isTrue()
            coVerify(exactly = 0) { exerciseLogDao.upsertExerciseLog(any()) }
            coVerify(exactly = 0) { setLogDao.upsertSetLog(any()) }
        }

    @Test
    fun `orphan detection - multiple orphans in batch are all filtered out`() =
        runTest {
            val userId = "test-user"
            val orphan1 =
                FirestoreExerciseLog(
            ...
            } returns Result.success(listOf(orphan1, orphan2, orphan3))
            mockAllFirestoreUploads()
