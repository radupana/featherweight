...
class ImportProgrammeViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun setParsedProgramme(programme: ParsedProgramme, parseRequestId: Long? = null) {
        _uiState.value = _uiState.value.copy(
    ...
    private fun processProgrammeAsync(requestId: Long) {
        viewModelScope.launch {
            try {
                ...
                val userMaxesMap = allMaxes.associate { 
                ...
                if (result.success && result.programme != null) {
                    ...
                    repository.updateParseRequest(
                        parseRequest.copy(
                            ...
                            resultJson = com.google.gson.Gson().toJson(matchedProgramme),
    ...
    private fun findBestExerciseMatch(
        ...
        val nameWithVariations = nameLower
        ...
        val nameWithoutEquipment = nameLower
            .replace("barbell ", "")
            .replace("dumbbell ", "")
            .replace("db ", "")
            .replace("cable ", "")
    ...
    fun updateParsedWorkout(weekIndex: Int, workoutIndex: Int, updatedWorkout: ParsedWorkout) {
    ...
    fun getParsedWorkout(weekIndex: Int, workoutIndex: Int): ParsedWorkout? {
        val programme = _uiState.value.parsedProgramme ?: return null
        return programme.weeks.getOrNull(weekIndex)?.workouts?.getOrNull(workoutIndex)
    }
    ...
    fun applyExerciseMappings() {
        ...
        _uiState.value = _uiState.value.copy(
            parsedProgramme = programme.copy(
                weeks = updatedWeeks,
    ...
    fun confirmAndCreateProgramme(onSuccess: () -> Unit = {}) {
        ...
        viewModelScope.launch {
            ...
            try {
                val programmeId = createProgrammeFromParsed(programme)
                _uiState.value = _uiState.value.copy(
                    isLoading = false,
                    parsedProgramme = null,
                    inputText = "",
    ...
    private suspend fun createProgrammeFromParsed(parsedProgramme: ParsedProgramme): Long {
        ...
        val difficulty = try {
            com.github.radupana.featherweight.data.programme.ProgrammeDifficulty.valueOf(parsedProgramme.difficulty)
        } catch (e: IllegalArgumentException) {
            com.github.radupana.featherweight.data.programme.ProgrammeDifficulty.INTERMEDIATE
        }
        
        val programmeId = repository.createImportedProgramme(
    ...
    private fun buildProgrammeJson(programme: ParsedProgramme): String {
        val weeks = programme.weeks.map { week ->
            mapOf(
        ...
        return com.google.gson.Gson().toJson(mapOf("weeks" to weeks))
    }
