...
interface VariationMuscleDao {
    ...
    suspend fun deleteSpecificMuscleMappingForVariation(variationId: Long, muscle: MuscleGroup)
    ...
    """)
    suspend fun getVariationIdsByMuscles(muscles: List<MuscleGroup>, muscleCount: Int): List<Long>
    
    @Query("UPDATE variation_muscles SET emphasisModifier = :modifier WHERE variationId = :variationId AND muscle = :muscle")
    suspend fun updateEmphasisModifier(variationId: Long, muscle: MuscleGroup, modifier: Float)
