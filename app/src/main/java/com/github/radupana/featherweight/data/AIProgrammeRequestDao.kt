...
interface AIProgrammeRequestDao {
    ...
    suspend fun saveGeneratedProgramme(
        id: String,
        json: String,
    ...
    suspend fun updateWorkManagerId(
        id: String,
        workManagerId: String,
    )

    @Query("SELECT * FROM ai_programme_requests WHERE status = :status AND createdAt < :beforeTimestamp")
    suspend fun getRequestsOlderThan(
        status: GenerationStatus,
        beforeTimestamp: Long,
    ): List<AIProgrammeRequest>

    @Query("DELETE FROM ai_programme_requests WHERE id = :id")
    suspend fun delete(id: String)

    @Query(
        "UPDATE ai_programme_requests SET status = :status, clarificationMessage = :clarificationMessage, lastUpdatedAt = :timestamp WHERE id = :requestId",
    )
    suspend fun updateStatusWithClarification(
        requestId: String,
        status: GenerationStatus,
