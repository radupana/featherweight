...
object TemplateFilters {
    ...
    fun matchesFilter(template: ExampleTemplate, filter: TemplateFilter): Boolean {
        return when (filter.category) {
            TemplateFilterCategory.GOAL -> when (filter.value) {
            ...
            TemplateFilterCategory.FREQUENCY -> when (filter.value) {
