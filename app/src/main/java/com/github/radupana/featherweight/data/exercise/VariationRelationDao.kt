...
interface VariationRelationDao {
    ...
    """)
    suspend fun getOutgoingRelationsByType(variationId: Long, type: ExerciseRelationType): List<VariationRelation>
    
    @Query("""
        SELECT * FROM variation_relations 
        WHERE toVariationId = :variationId AND relationType = :type
    """)
    suspend fun getIncomingRelationsByType(variationId: Long, type: ExerciseRelationType): List<VariationRelation>
    ...
    """)
    suspend fun getRelatedVariations(variationId: Long, type: ExerciseRelationType): List<ExerciseVariation>
    ...
    @Query("""
        ...
        LIMIT 1
    """)
    suspend fun getRelationBetween(fromId: Long, toId: Long): VariationRelation?
    ...
    suspend fun updateRelationStrength(id: Long, strength: Float)
}
