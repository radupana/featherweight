...
class FeatherweightSuggestionEngine(
    ...
    override suspend fun suggestWeight(exerciseName: String, targetReps: Int): WeightSuggestion {
        ...
        if (oneRM != null) {
            ...
            sources.add(SuggestionSourceData(
        ...
        val finalWeight = if (weights.isNotEmpty()) {
        ...
        val finalConfidence = if (confidences.isNotEmpty()) {
    ...
    override suspend fun suggestReps(exerciseName: String, targetWeight: Float): RepsSuggestion {
        ...
        // 1. One RM calculation (if available)
        val oneRM = repository.getOneRMForExercise(exerciseName)
        val suggestedReps = if (oneRM != null && oneRM > 0) {
    ...
    override suspend fun explainSuggestion(exerciseName: String, suggestion: WeightSuggestion): String {
        return suggestion.explanation
    }
    
    override fun observeWeightSuggestions(exerciseName: String, targetReps: Flow<Int>): Flow<WeightSuggestion> {
        return targetReps.map { reps ->
            suggestWeight(exerciseName, reps)
        }
    }
    
    override fun observeRepsSuggestions(exerciseName: String, targetWeight: Flow<Float>): Flow<RepsSuggestion> {
    ...
    private fun calculateRepsForWeight(targetWeight: Float, oneRM: Float): Int {
    ...
    private fun calculateTrendWeight(recentSets: List<PerformanceData>, targetReps: Int): Float {
        if (recentSets.isEmpty()) return 0f
        
        // Simple linear progression calculation
        val relevantSets = recentSets.filter { 
        ...
        return if (relevantSets.isNotEmpty()) {
            ...
            val successRate = relevantSets.count { 
    ...
    private suspend fun calculateCorrelatedWeight(exerciseName: String, targetReps: Int): Float? {
    ...
    private fun calculateWeightedAverage(values: List<Float>, weights: List<Float>): Float {
    ...
    private fun generateAlternativeWeights(sources: List<SuggestionSourceData>, currentReps: Int): Map<Int, Float> {
        if (sources.isEmpty()) return emptyMap()
        
        val baseWeight = sources.firstOrNull { it.source == SuggestionSource.ONE_RM_CALCULATION }?.value
    ...
    private fun generateAlternativeReps(targetWeight: Float, oneRM: Float?): Map<Float, Int> {
    ...
    private fun buildExplanation(sources: List<SuggestionSourceData>, finalWeight: Float, confidence: Float): String {
        if (sources.isEmpty()) {
