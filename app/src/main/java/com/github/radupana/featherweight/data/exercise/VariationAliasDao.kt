...
interface VariationAliasDao {
    ...
    """)
    suspend fun getAliasesByLanguage(variationId: Long, languageCode: String): List<VariationAlias>
    
    @Query("""
        SELECT * FROM variation_aliases 
        WHERE variationId = :variationId AND source = :source
    """)
    suspend fun getAliasesBySource(variationId: Long, source: String): List<VariationAlias>
    ...
    suspend fun updateAliasConfidence(id: Long, confidence: Float)
